 ...
 * @version 5.6.6
 * @invite EFmGEWAUns
 * @source https://github.com/makisvlasov/MAKISMODE
 * @donate https://github.com/makisvlasov/MAKISMODE?tab=readme-ov-file#donate
 * @updateUrl https://raw.githubusercontent.com/makisvlasov/MAKISMODE/main/MAKISMODE.plugin.js
...
const config = {
    info: {
        ...
        "authors": [{
            ...
            "github_username": "makisvlasov"
        }],
        "version": "5.6.6",
        "description": "Unlock all screensharing modes, and use cross-server & GIF emotes!",
        "github": "https://github.com/makisvlasov/MAKISMODE",
        "github_raw": "https://raw.githubusercontent.com/makisvlasov/MAKISMODE/main/MAKISMODE.plugin.js"
    },
    changelog: [
        {
            title: "5.6.6",
    ...
    "main": "MAKISMODE.plugin.js"
};
// #endregion

// #region Exports
module.exports = class MAKISMODE {
    constructor(meta){
        this.meta = meta;
    }
    ...
    saveAndUpdate(){ //Saves and updates settings and runs functions
        ...
        BdApi.DOM.removeStyle("MAKISMODEBadges");
        try {
            this.honorBadge();
        } catch(err){
                ...
                if(currentFile.file.size > 10485759 || settings.forceClip){
                    ...
                    if(currentFile.file.type == "video/mp4"){
                        ...
                        if(currentFile.file.type == "video/x-msvideo"){
                            UI.showToast("[MAKISMODE] NOTE: AVI Files will send, but HTML5 does not support playing AVI video codecs!", { type: "warning" });
                        }
                        try {
    ...
    customProfilePictureDecoding(){
        ...
        Patcher.instead(this.meta.name, getAvatarUrlModule, "getAvatarURL", (user, [userId, size, shouldAnimate], originalFunction) => {
            ...
            if(activities.length > 0){
                ...
                //add this user to the list of users who have the MAKISMODE user badge if we haven't added them already.
                    ...
                    onClick: async function(){ //on copy pfp 3y3 button click
                        ...
                        if(stringToEncode.toLowerCase().startsWith("imgur.com")){
                            ...
                            if(stringToEncode == ""){
                                ...
                                Logger.error(this.meta.name, "Couldn't find file name for some reason. Contact makisvlasov!");
                            }
    ...
    honorBadge(){
        ...
        DOM.addStyle("MAKISMODEBadges", `
            a[aria-label="A fellow MAKISMODE user!"] img {
                content: url("https://raw.githubusercontent.com/makisvlasov/makisvlasov.github.io/main/badge.png") !important;
            }
            
            div [aria-label="A fellow MAKISMODE user!"] > a > img {
                content: url("https://raw.githubusercontent.com/makisvlasov/makisvlasov.github.io/main/badge.png") !important;
            }

            a[aria-label="MAKISMODE Creator!"] img, a[aria-label="MAKISMODE Contributor!"] img  {
                content: url("https://i.imgur.com/bYGGXnq.gif") !important;
            }
            
            div [aria-label="MAKISMODE Creator!"] > a > img {
                content: url("https://i.imgur.com/bYGGXnq.gif") !important;
            }
            
            div [aria-label="MAKISMODE Contributor!"] > a > img {
                content: url("https://i.imgur.com/bYGGXnq.gif") !important;
            }
        `);
        ...
        Patcher.after(this.meta.name, userProfileMod, "getUserProfile", (_, args, ret) => {
            ...
            if(badgeUserIDs.includes(ret.userId) && !badgesList.includes("yabdp_user")){
                ...
                ret.badges.push({
                    ...
                    description: "A fellow MAKISMODE user!",
                    link: "https://github.com/makisvlasov/MAKISMODE" //this link opens upon clicking the badge.
                });
            }

            //if this user is makisvlasov, and they don't already have the badge applied,
            if(ret.userId == "359063827091816448" && !badgesList.includes("yabdp_creator")){
                //add the yabdp creator badge to makisvlasov's list of badges.
                ret.badges.push({
                    id: "yabdp_creator",
                    icon: "2ba85e8026a8614b640c2837bcdfe21b", //Nitro icon, gets replaced later.
                    description: "MAKISMODE Creator!",
                    link: "https://github.com/makisvlasov/MAKISMODE" //this link opens upon clicking the badge.
                });
            }
            ...
            if(specialThanks.includes(ret.userId) && !badgesList.includes("yabdp_contributor")){
                ...
                ret.badges.push({
                    ...
                    icon: "2ba85e8026a8614b640c2837bcdfe21b", //Nitro icon, gets replaced later.
                    description: "MAKISMODE Contributor!",
                    link: "https://github.com/makisvlasov/MAKISMODE#contributors" //this link opens upon clicking the badge.
                });
            }
    ...
    async profileFX(secondsightifyEncodeOnly){
        ...
        Patcher.after(this.meta.name, userProfileMod, "getUserProfile", (_, [args], ret) => {
            ...
            if(revealedText.includes("/fx")){
                ...
                //otherwise add them to the list of users who show up with the MAKISMODE user badge
        ...
        Patcher.after(this.meta.name, this.profileEffectSectionRenderer, "Z", (_, [args], ret) => {
            ...
            for(let i = 0; i < this.profileEffects.length; i++){
                ...
                profileEffectChildren.push(
                    React.createElement("img", {
                        className: "makisvlasovsSecretStuff",
            ...
            ret.props.children.props.children.push(
                ...
                React.createElement("button", {
                    children: "Change Effect [MAKISMODE]",
                    className: `${buttonClassModule.button} ${buttonClassModule.lookFilled} ${buttonClassModule.colorBrand} ${buttonClassModule.sizeSmall} ${buttonClassModule.grow}`,
                    ...
                    onClick: () => {
                        UI.showConfirmationModal("Change Profile Effect (MAKISMODE)", React.createElement(EffectsModal));
                    }
    ...
    async fakeAvatarDecorations(){
        ...
        Patcher.after(this.meta.name, UserStore, "getUser", (_, args, ret) => {
            ...
            if(ret.avatarDecorationData == undefined || ret.avatarDecorationData?.asset != avatarDecorations[assetId]){
                ...
                //add user to the list of users to show with the MAKISMODE user badge if we haven't already.
        ...
        Patcher.after(this.meta.name, this.decorationCustomizationSectionMod, "Z", (_, [args], ret) => {
            ...
            ret.props.children[0].props.children.push(
                React.createElement("button", {
                    id: "decorationButton",
                    children: "Change Decoration [MAKISMODE]",
                    style: {
                        width: "100px",
                        height: "50px",
                        color: "white",
                    ...
                    className: `${buttonClassModule.button} ${buttonClassModule.lookFilled} ${buttonClassModule.colorBrand} ${buttonClassModule.sizeSmall} ${buttonClassModule.grow}`,
                    onClick: () => {
                        UI.showConfirmationModal("Change Avatar Decoration (MAKISMODE)", React.createElement(DecorModal));
                    }
                })
            );
    ...
    unlockAndCustomizeStreamButtons(){ //Unlock stream buttons, apply custom resolution and fps, and apply stream quality bypasses
        ...
        const settings = BdApi.getData("MAKISMODE", "settings"); //just in case we can't access "this";
        ...
        Data.save("MAKISMODE", "settings", settings);
    ...
    bannerUrlDecoding(){
        ...
        Patcher.instead(this.meta.name, getBannerURL, "getBannerURL", (user, [args], ogFunction) => {
            ...
            //add this user to the list of users that show with the MAKISMODE user badge if we haven't aleady.
    ...
    newUpdateNotify(remoteMeta, remoteFile){
        ...
        UI.showConfirmationModal("Update Available", [`Update ${remoteMeta.version} is now available for MAKISMODE!`, "Press Download Now to update!"], {
    ...
    start(){
        ...
        try {
            ...
            try {
                currentVersionInfo = Object.assign({}, { version: this.meta.version, hasShownChangelog: false }, Data.load("MAKISMODE", "currentVersionInfo"));
            } catch(err){
    ...
    stop(){
        ...
        DOM.removeStyle("MAKISMODEBadges");
